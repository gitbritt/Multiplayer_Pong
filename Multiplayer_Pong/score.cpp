...
void GUI::initialize_scores(sf::Text &Score1, sf::Text &Score2, sf::Font &font, int player_number)
{
	...
	Score1.setString(player_str += std::to_string(Score1_num));						//Displays Score for player 1
	Score2.setString(player_str += std::to_string(Score2_num));						//Displays Score for player 2
